function rrtPath = rrtPlanner(startPos,goalPos,displayPath,field)
arguments
    ...
    field.obstacles (:,4) double = [1,0,0,0.1] % [ID, Cx, Cy, S]
    field.wsBounds (2,2) double = [0,50;0,50]  % [xmin, xmax; ymin, ymax]
end % arg
...
while(looping && currNode < maxNodes)
    ...
    % future.
    newTarget = [(field.wsBounds(1,2)-field.wsBounds(1,1))*rand()+field.wsBounds(1,1), ...
        (field.wsBounds(2,2)-field.wsBounds(2,1))*rand()+field.wsBounds(2,1),...
    ...
    % rejected.
    collisionFlg = checkCollide(newNode(1:2),gTree(bestNode,1:2),field.obstacles,field.wsBounds);
    if(~collisionFlg)
        ...
        while(looping2)
            ...
            elseif(~checkCollide(neighborRadius(newParentIdx,2:3),newNode(1:2),field.obstacles,field.wsBounds))
                looping2 = false;
            else
        ...
        if(~collisionFlg)
            ...
            for idx = 1:size(neighborRadius,1)
                ...
                if(neighborRadius(idx,4) > propScore && ...
                        ~checkCollide(neighborRadius(idx,2:3),newNode(1:2),field.obstacles,field.wsBounds))
...
if(displayPath)
    ...
    for k = 1:size(field.obstacles,1)
        rectangle('Position', [field.obstacles(k,2)-(field.obstacles(k,4)/2),field.obstacles(k,3)-(field.obstacles(k,4)/2),field.obstacles(k,4),field.obstacles(k,4)],...
